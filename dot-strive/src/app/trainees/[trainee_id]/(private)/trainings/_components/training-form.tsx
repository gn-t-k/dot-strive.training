...
export const TrainingForm: FC<Props> = (props) => {
  ...
  const onClickAddExercise: MouseEventHandler<HTMLButtonElement> = (_) => {
    append({
      exerciseId: "",
      sets: [],
      memo: "",
    ...
    >
      ...
      <div className={stack({ direction: "column" })}>
        ...
        {fields.map((record, index) => {
          ...
          return (
            <div className={stack({ direction: "row" })} key={record.id}>
              <RecordForm
                control={control}
                recordIndex={index}
                register={register}
              ...
              />
              <Button onClick={onClickRemoveRecord}>記録を削除</Button>
            </div>
          );
        })}
...
const RecordForm: FC<RecordFormProps> = (props) => {
  ...
  const onClickAddSet: MouseEventHandler<HTMLButtonElement> = (_) => {
    append({
      weight: "",
      repetition: "",
    });
        ...
        {fields.map((set, index) => {
          ...
          return (
            <div className={stack({ direction: "row" })} key={set.id}>
              ...
              />
              <Button onClick={onClickRemoveSet}>セットを削除</Button>
            </div>
          );
        })}
