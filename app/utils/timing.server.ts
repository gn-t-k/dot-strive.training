...
export const time = async <T>(
  ...
  try {
    ...
    recordServerTiming(serverTimings, {
      name,
      description: "Error",
    });
...
const recordServerTiming = (
  ...
  const serverTiming: PerformanceServerTiming = {
    ...
    // biome-ignore lint/style/useNamingConvention: PerformanceServerTimingで定義されているため
    toJSON: () => {
...
export const getServerTimeHeaderField = (
  serverTimings: PerformanceServerTimings,
) => {
